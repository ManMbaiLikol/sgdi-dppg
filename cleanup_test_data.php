...
$sql = "SELECT id, numero, nom_demandeur, ville, region, type_infrastructure, observations, date_creation
        FROM dossiers
        WHERE observations LIKE '%Test pilote%' OR observations LIKE '%test%' OR observations LIKE '%pilote%'
        ORDER BY id";
...
echo "<h3>⚠️ " . count($test_dossiers) . " dossiers de test trouvés</h3>";
...
foreach ($test_dossiers as $d) {
    ...
    echo "<td>" . htmlspecialchars($d['numero']) . "</td>";
    echo "<td>" . htmlspecialchars($d['nom_demandeur']) . "</td>";
    echo "<td>" . htmlspecialchars($d['type_infrastructure']) . "</td>";
    echo "<td>" . htmlspecialchars($d['ville']) . "</td>";
    echo "<td>" . htmlspecialchars($d['region']) . "</td>";
...
foreach ($related_tables as $table => $query) {
    try {
        ...
        echo "<tr><td>$table</td><td><em>Table non trouvée</em></td></tr>";
    }
}
...
echo "<h3>📝 Résumé</h3>";
echo "<ul>";
echo "<li><strong>" . count($test_dossiers) . " dossiers</strong> de test à supprimer</li>";
echo "<li><strong>$total_related enregistrements liés</strong> dans d'autres tables</li>";
...
if (!isset($_POST['confirm_delete'])) {
    ...
    echo "<p>Êtes-vous sûr de vouloir supprimer ces " . count($test_dossiers) . " dossiers de test et toutes leurs données liées?</p>";
    echo "</div>";
    ...
    try {
        ...
        foreach ($delete_queries as $table => $query) {
            try {
                ...
                echo "<tr><td>$table</td><td><em>Erreur: " . $e->getMessage() . "</em></td></tr>";
            }
        }
        ...
        echo "<p><strong>" . $deleted_counts['dossiers'] . " dossiers de test</strong> ont été supprimés de la base de données.</p>";
