...
try {
    ...
    $table_decisions = 'decisions_ministerielle'; // Par défaut
    $stmt_check = $pdo->query("SHOW TABLES LIKE 'decisions_ministerielle'");
    if ($stmt_check->rowCount() == 0) {
        // Si decisions_ministerielle n'existe pas, essayer 'decisions'
        $stmt_check = $pdo->query("SHOW TABLES LIKE 'decisions'");
        if ($stmt_check->rowCount() > 0) {
            $table_decisions = 'decisions';
        } else {
            $table_decisions = null; // Aucune table de décisions
        }
    }
    ...
    $sql .= $table_decisions ? " ORDER BY dec.date_decision DESC, d.numero DESC" : " ORDER BY d.date_creation DESC, d.numero DESC";
